...
var RevealMenu = window.RevealMenu || (function(){
	...
	function loadPlugin() {
		...
		if (initialise) {
			...
			var themes = select('link#theme') ? options.themes : true;
			console.log(themes)
			if (themes === true) {
				themes = [
			...
			var transitions = options.transitions || true;
			if (transitions === true) {
			...
			function init() {
				if (!initialised) {
					...
					var button = create('li', {id: 'close', 'class': 'toolbar-panel-button'});
